...
describe("endpoint : Locations [8.2.2]", () => {
    ...
    describe(`{get} ${entity.name} ${nbColorTitle}[9.2]`, () => {
        it(`Return all ${entity.name} ${nbColor}[9.2.2]`, (done) => {
            ...
            executeQuery(count(entity.table)).then((result) => {
                    chai.request(server)
                        ...
                        .end((err, res) => {
                            const nb = Number(result["count"]) > 200 ? 200 : Number(result["count"]);
        ...
        it(`Return all ${entity.name} of a specific Thing ${nbColor}[9.2.6]`, (done) => {
            executeQuery(`${count("thing_location")} WHERE "thing_id" = 5`)
                .then((countThingLocation) => {
                    const infos = {
                        ...
                        apiExample: {
                            http: `/${testVersion}/Things(5)/${entity.name}`,
                            curl: defaultGet("curl", "KEYHTTP"),
                            javascript: defaultGet("javascript", "KEYHTTP"),
                            python: defaultGet("python", "KEYHTTP")
                        },
                    ...
                    chai.request(server)
                        ...
                        .end((err, res) => {
                            ...
                            res.status.should.equal(200);
                            res.type.should.equal("application/json");
                            res.body.value.length.should.eql(countThingLocation["count"]);
                            res.body.should.include.keys("@iot.count", "value");
                            addToApiDoc({ ...infos, result: limitResult(res) });
                            done();
                        });
        ...
        it(`Return ${entity.name} Subentity Things ${nbColor}[9.2.6]`, (done) => {
            const name = "Things";
            chai.request(server)
                .get(`/test/${testVersion}/${entity.name}(1)/Things`)
                .end((err: Error, res: any) => {
                    should.not.exist(err);
                    res.status.should.equal(200);
                    res.type.should.equal("application/json");
    ...
    describe(`{post} ${entity.name} ${nbColorTitle}[10.2]`, () => {
        ...
        it(`Return Error if the payload is malformed ${nbColor}[10.2.2]`, (done) => {
            chai.request(server)
                .post(`/test/${testVersion}/locations`)
                .send({})
                .set("Cookie", `${keyTokenName}=${token}`)
                .end((err: Error, res: any) => {
                    should.not.exist(err);
