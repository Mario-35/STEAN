 ...
 */
 ...
 const datastream = (nb: number) =>
     `WITH datastream as (insert into "datastream" ("description", "name", "observedproperty_id", "sensor_id", "thing_id", "unitOfMeasurement") values ('Air quality Number ${
         numberStr[nb]
 ...
 export const datasDemo = (): string[] => {
     const result: string[] = [    
     `WITH decoder as (insert into "decoder" ("name", "code", "test") values ('RHF1S001', 'const nomenclature = { "voltage": "battery voltage", "period": "periods", "0110": "air temperature", "0210": "air humidity", "0310": "light intensity", "0410": "humidity", "0510": "barometric pressure", "0610": "soil temperature", "0700": "battery", "0710": "soil moisture" }; const decoded = { valid: true, err: 0, payload: input, messages: [] }; const temp = input.match(/.{1,2}/g); if (temp != null) { if (temp[0] == "01" || temp[0] == "81") { decoded.messages.push({ type: "report_telemetry", measurementName: nomenclature["0610"], measurementValue: (parseInt(String(temp[2]) + String(temp[1]), 16) * 175.72) / 65536 - 46.85 }); decoded.messages.push({ type: "report_telemetry", measurementName: nomenclature["0710"], measurementValue: (parseInt(temp[3], 16) * 125) / 256 - 6 }); decoded.messages.push({ type: "upload_battery", measurementName: nomenclature["period"], measurementValue: parseInt(String(temp[5]) + String(temp[4]), 16) * 2 }); decoded.messages.push({ type: "upload_battery", measurementName: nomenclature["voltage"], measurementValue: (parseInt(temp[8], 16) + 150) * 0.01 }); return decoded; } } decoded["valid"] = false; decoded["err"] = -1; return decoded;', '012f5ecec2014a1ab2')RETURNING *)SELECT * FROM decoder;`,
     `WITH decoder as (insert into "decoder" ("name", "code", "test") values ('NKE_LevelO_4F06', 'if (typeof input === "object") { let decoded = {}; var name = input["columns"]; var values = input["values"][0]; name.forEach((elem, index) => { decoded[elem] = values[index]; }); return {value: decoded["value"]}; } return {};', '{ "data": { "values": [ [ "Measure", "Multi_Fluid|0|IAnalogGenericRawValue", 4.8045387268066 ] ], "columns": [ "type", "ref", "value" ] }, "deveui": "70b3d5e75e014f06", "datetime": "2023-02-09T12:10:31+00:00", "device_id": "1755", "timestamp": "1675944631", "device_name": "NKE_LevelO_4F06", "customElement": [], "payload_deciphered": "110a000c0055394099bec8" }')RETURNING *)SELECT * FROM decoder;`
 ];
    try {
