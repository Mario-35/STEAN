...
import { DEFAULT_API_VERSION, ESCAPE_ARRAY_JSON, ESCAPE_SIMPLE_QUOTE, TEST } from "../constants";
import { log } from "../log";
          ...
          Observations: {
            ...
            columns: {
              ...
              payload: {
                create: "",
                columnAlias() {
                  return `CASE WHEN result->'payload' notnull THEN result->>'payload' WHEN result->'valueskeys' notnull THEN result->>'valueskeys' WHEN result->'value' notnull THEN result->>'value' END AS payload`; },
                type: "string",
              },
              deveui: {
                create: "",
                columnAlias() {
                  return `CASE WHEN multidatastream_id notnull  THEN (select deveui from lora where multidatastream_id = observation.multidatastream_id) WHEN datastream_id notnull  THEN (select deveui from lora where datastream_id = observation.datastream_id) END AS deveui`;
                },
                type: "string",
              },
