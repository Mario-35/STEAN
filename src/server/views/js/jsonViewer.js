 ...
 function isCollapsable(arg) {
  ...
  function json2html(json, options) {
    ...
    if (typeof json === 'string') {
      ...
      if (isUrl(tmp)) {
        html += `<span class="json-url">${tmp}</span>`;
      } else if (isCode(tmp)) {
        html += `<span class="json-code">${tmp}</span>`;
      } else {
   ...
   */
  function jsonViewer(json, container, opt) {
    ...
    off('click', target);
    target.addEventListener('click', ToggleEventListener);
    target.addEventListener('click', SimulateClickEventListener);
