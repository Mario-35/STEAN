...
export const  queryAsDataArray = (listOfKeys: { [key: string]: string } , query: string, singular: boolean, fields?: string[]): string => {  
    const sqlString = `SELECT (ARRAY['${Object.keys(listOfKeys).map((e:string) => removeQuotes(e)).join("','")}']) as "component", jsonb_agg(allkeys) as "dataArray" FROM (SELECT  json_build_array(${Object.values(listOfKeys).join()}) as allkeys FROM (${query}) as p) as l`;
