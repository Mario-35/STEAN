...
export function createpgQuery(main: PgVisitor, element: PgVisitor): IpgQuery | undefined { 
    ...
    if (realEntity) {
        ...
        if (select) {
            ...
            if (realEntityName) {
            const relations: string[] = Object.keys(DBDATAS[realEntityName].relations);                      
                element.includes.forEach((item) => {                                
                    const name = item.navigationProperty;                                                
                    const index = relations.indexOf(name);
                    if (index >= 0) {
                        item.setEntity(name);
                        item.where += `${item.where.trim() == "" ? '' : " AND "}${DBDATAS[realEntityName].relations[name].expand}`;                                                            
                ...
                // relations.filter((rel: string) => main.config.dbEntities.includes(DBDATAS[realEntityName].relations[rel].entityName)).forEach((rel: string) => { 
                ...
                relations.forEach((rel: string) => {                    
                    if (rel[0] == "(") select.push(rel);
                        ...
                        if (temTable) {
                            select.push(`CONCAT('${main.options.rootBase}${DBDATAS[realEntityName].name}(', "${DBDATAS[realEntityName].table}"."id", ')/${rel}') AS "${rel}@iot.navigationLink"`);
                            main.addToBlanks(`'${main.options.rootBase}${DBDATAS[realEntityName].name}(0)/${rel}' AS "${rel}@iot.navigationLink"`);
                        }
                    }
                });
                return { 
                    select: select.join(",\n\t"), 
                    from: DBDATAS[realEntityName].table , 
                    where: element.where, 
