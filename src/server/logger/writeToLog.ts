...
import { createInsertValues, executeSqlValues } from "../db/helpers";
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const writeToLog = async ( ctx: koa.Context, ...error: any[] ): Promise<void> => {
  Logs.whereIam();
  if (error.length > 0) Logs.writeErrorInFile(ctx, error);  
  if (ctx._log && ctx._log.method != "GET") {
    ...
    await executeSqlValues(ctx._config.name, `INSERT INTO "${_DB.Logs.table}" ${createInsertValues(ctx._log, _DB.Logs.name)} returning id`).then((res: object) =>{
