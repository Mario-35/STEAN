...
import fs from "fs";
import { ADMIN, API_VERSION, APP_NAME, APP_VERSION, NODE_ENV, setDebug, setReady, TEST, TIMESTAMP } from "../constants";
...
class Configuration {
    ...
    async createDBTest() { 
        ...
        this.formatConfig(Configuration.jsonConfiguration[TEST], TEST);
        return returnValue;
    }
    ...
    async start (): Promise<void> {
        isTest() ? await this.initTest() : await this.init();
        setReady(true);
    }

    async removeTests(): Promise<boolean> {
        return await this.db(ADMIN).raw(`SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'test';`).then(async () => {
            await this.db(ADMIN).raw("DROP DATABASE IF EXISTS test");
            return true;
        });
    }
    ...
    private formatConfig(input: object | string, name?: string): IconfigFile {
        ...
        const returnValue: IconfigFile = {
            name: goodDbName,
            port: goodDbName === "admin" ? input["port"] || 8029 : this.configs[ADMIN].port,
            pg: {
    ...
    async addConfig(addJson: object): Promise<IconfigFile> {
        ...
        input[configs.name] = configs;
         ...
         fs.writeFile(Configuration.filePath, JSON.stringify(input, null, 4), err => {
          ...
          this.configs[configs.name] = configs;
        //   await this.addToServer(app , configs.name);
    ...
    async addToServer(key: string): Promise<boolean> {
        await this.isDbExist(key, true)
            .then(async (res: boolean) => {
              ...
              .catch((error: Error) => {
                  Logs.error(messages.errors.unableFindCreate, this.configs[key].pg.database);
                  console.log(error);
                  process.exit(111);
              });
              return false;
    ...
    async isDbExist(connectName: string, create: boolean): Promise<boolean> {
        Logs.booting(messages.infos.dbExist, this.configs[connectName].pg.database);
            ...
            .catch(async (err: Error) => {
                ...
                if (err["code"] === "28P01" ) {
                   console.log("error connection");                   
                    ...
                    Logs.debug(messagesReplace(messages.infos.tryCreate, [messages.infos.db]), this.configs[connectName].pg.database);
                    returnResult = await createDatabase(connectName)
                        .then(async () => {
                             Logs.result(`${messages.infos.db} ${messages.infos.create} [${this.configs[connectName].pg.database}]`, "OK");
